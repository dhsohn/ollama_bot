...
- 피드백 수집 (👍/👎 + 사유)
- 계층형 응답: 스킬 → 즉시응답 → 인텐트라우팅 → 시맨틱캐시 → Full LLM(planner + reviewer + RAG)
- reviewer issue 누적 관측: rewrite 빈도와 주요 품질 이슈를 feedback 통계에 축적
- 장기 메모리/대화 보관 (SQLite)
...
cp config/config.yaml.example config/config.yaml   # 토큰/ID 설정
```

### 설정 (`config/config.yaml`)

모든 설정을 단일 YAML 파일에서 관리한다. 시크릿 포함이므로 `.gitignore` 대상.
...
- `telegram.allowed_users`: 숫자 Chat ID (쉼표 구분). placeholder 상태면 시작 시 종료됨.
...
| Orchestration | `core/engine.py`, `core/engine_*` | 계층형 라우팅, 컨텍스트 조립, 모델 준비, 요청 추적, planner/reviewer/RAG/요약 orchestration |
...
| Operations | `core/auto_scheduler.py`, `core/automation_callables*.py` | 자동화 실행, 운영형 작업 orchestration |
...
| `core/engine_planner.py` | Full-tier 내부 1차 응답 설계(JSON planner)와 scaffold 주입 |
| `core/engine_reviewer.py` | draft answer 검수, JSON review, 필요 시 rewrite 적용 |
...
- 줄 수가 아니라 책임 경계로 분리한다. 예: `telegram`은 `commands/messages/feedback`, `engine`은 `routing/context/summary/rag/tracking`으로 나눈다.
...
| `scripts/setup.sh` | 초기 설정 (`config.yaml`, 디렉터리, 모델 점검) |
