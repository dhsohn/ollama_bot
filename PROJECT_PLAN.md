...
├── README.md
├── docker-compose.yml       # 단일 배포 compose
├── Dockerfile               # 공용 컨테이너 빌드 정의
├── .env.example             # 환경변수 템플릿
├── .dockerignore
├── requirements.txt         # 런타임 의존성
├── requirements-dev.txt     # 테스트/개발 의존성
├── apps/
│   └── ollama_bot/main.py   # 단일 앱 진입점
├── config/
│   └── config.yaml          # 봇 전역 설정
├── core/                    # shared core
│   ├── __init__.py
...
│   ├── lemonade_client.py   # Lemonade API 클라이언트
...
│   └── security.py          # 보안 모듈
├── packages/
│   └── hw_amd_npu/          # AMD NPU 런타임 프로파일 모듈
...
- 단일 앱(`ollama_bot`)에서 `.env`의 `LLM_PROVIDER`로 백엔드를 선택
- `LLM_PROVIDER=ollama`면 `OLLAMA_*` 설정 사용
- `LLM_PROVIDER=lemonade`면 `LEMONADE_*` 설정 사용
- `AMD_NPU_PROFILE`로 NPU 런타임 스레드 기본값을 보정(선택)

### 8.3 Dockerfile 구성 방향
- Python 3.11-slim 기반 (경량 이미지)
- 단일 이미지 + 단일 앱 entrypoint
- non-root 사용자 실행
- 최소 패키지만 설치
...
### 10.6 CI 운영 원칙
- `ruff`, `mypy`, `pytest`를 단일 파이프라인에서 항상 실행
- `docker-compose.yml` 기준으로 이미지 빌드 검증 수행
- 앱 분리를 전제로 한 경로 분기 CI는 사용하지 않음
...
LEMONADE_HOST=http://host.docker.internal:8000
LEMONADE_MODEL=
LEMONADE_API_KEY=
LEMONADE_BASE_PATH=/api/v1
LEMONADE_TIMEOUT_SECONDS=60
